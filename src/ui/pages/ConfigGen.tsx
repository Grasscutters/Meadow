...
import { MailItem, Region, Config, RunMode, ServerDebugMode, LogLevel } from "@app/types";
...
class ConfigGen extends React.Component<{}, IState> {
        ...
        if (type === "MailItem") {
            this.setState({
                mailItems: [
                    ...
                    {
                        itemId: parseInt((document.getElementById("config-add-item-id") as HTMLInputElement)?.value || "0"),
                        itemCount: parseInt((document.getElementById("config-add-item-count") as HTMLInputElement)?.value || "0"),
                        itemLevel: parseInt((document.getElementById("config-add-item-level") as HTMLInputElement)?.value || "0")
                    }
                ]
            })
        } else {
            this.setState({
                regions: [
                    ...this.state.regions,
                    {
                        Name: (document.getElementById("config-add-region-name") as HTMLInputElement)?.value || "",
                        Title: (document.getElementById("config-add-region-title") as HTMLInputElement)?.value || "",
                        Port: parseInt((document.getElementById("config-add-region-port") as HTMLInputElement)?.value || "0"),
                        Ip: (document.getElementById("config-add-region-ip") as HTMLInputElement)?.value || ""
                    }
                ]
            })
        }
    ...
    private prepareConfig = async (): Promise<Config> => {
        ...
        config.folderStructure.resources = (document.getElementById("structure.resources") as HTMLInputElement)?.value || config.folderStructure.resources;
        config.folderStructure.data = (document.getElementById("structure.data") as HTMLInputElement)?.value || config.folderStructure.data;
        config.folderStructure.packets = (document.getElementById("structure.packets") as HTMLInputElement)?.value || config.folderStructure.packets;
        config.folderStructure.scripts = (document.getElementById("structure.scripts") as HTMLInputElement)?.value || config.folderStructure.scripts;
        config.folderStructure.plugins = (document.getElementById("structure.plugins") as HTMLInputElement)?.value || config.folderStructure.plugins;

        config.databaseInfo.server.connectionUri = (document.getElementById("database.server.connectionUri") as HTMLInputElement)?.value || config.databaseInfo.server.connectionUri;
        config.databaseInfo.server.collection = (document.getElementById("database.server.collection") as HTMLInputElement)?.value || config.databaseInfo.server.collection;
        config.databaseInfo.game.connectionUri = (document.getElementById("database.game.connectionUri") as HTMLInputElement)?.value || config.databaseInfo.game.connectionUri;
        config.databaseInfo.game.collection = (document.getElementById("database.game.collection") as HTMLInputElement)?.value || config.databaseInfo.game.collection;

        config.language.language = (document.getElementById("language.language") as HTMLInputElement)?.value || config.language.language;
        config.language.fallback = (document.getElementById("language.fallback") as HTMLInputElement)?.value || config.language.fallback;
        config.language.document = (document.getElementById("language.document") as HTMLInputElement)?.value || config.language.document;
        ...
        config.server.http.bindAddress = (document.getElementById("server.http.bindAddress") as HTMLInputElement)?.value || config.server.http.bindAddress;
        config.server.http.bindPort = parseInt((document.getElementById("server.http.bindPort") as HTMLInputElement)?.value) || config.server.http.bindPort;
        config.server.http.accessAddress = (document.getElementById("server.http.accessAddress") as HTMLInputElement)?.value || config.server.http.accessAddress;
        config.server.http.accessPort = parseInt((document.getElementById("server.http.accessPort") as HTMLInputElement)?.value) || config.server.http.accessPort;
        config.server.http.encryption.useEncryption = (document.getElementById("server.http.encryption.useEncryption") as HTMLInputElement)?.checked || config.server.http.encryption.useEncryption;
        config.server.http.encryption.useInRouting = (document.getElementById("server.http.encryption.useInRouting") as HTMLInputElement)?.checked || config.server.http.encryption.useInRouting;
        ...
        config.server.game.bindAddress = (document.getElementById("server.game.bindAddress") as HTMLInputElement)?.value || config.server.game.bindAddress;
        config.server.game.bindPort = parseInt((document.getElementById("server.game.bindPort") as HTMLInputElement)?.value) || config.server.game.bindPort;
        config.server.game.accessAddress = (document.getElementById("server.game.accessAddress") as HTMLInputElement)?.value || config.server.game.accessAddress;
        config.server.game.accessPort = parseInt((document.getElementById("server.game.accessPort") as HTMLInputElement)?.value) || config.server.game.accessPort;
        config.server.game.loadEntitiesForPlayerRange = parseInt((document.getElementById("server.game.loadEntitiesForPlayerRange") as HTMLInputElement)?.value) || config.server.game.loadEntitiesForPlayerRange;
        config.server.game.enableScriptInBigWorld = (document.getElementById("server.game.enableScriptInBigWorld") as HTMLInputElement)?.checked || config.server.game.enableScriptInBigWorld;
        config.server.game.enableConsole = (document.getElementById("server.game.enableConsole") as HTMLInputElement)?.checked || config.server.game.enableConsole;
        config.server.game.kcpInterval = parseInt((document.getElementById("server.game.kcpInterval") as HTMLInputElement)?.value) || config.server.game.kcpInterval;
        config.server.game.logPackets = (document.getElementById("server.game.logPackets") as HTMLSelectElement)?.options[(document.getElementById("server.game.logPackets") as HTMLSelectElement)?.selectedIndex ?? 0]?.value as ServerDebugMode || config.server.game.logPackets;
        config.server.game.isShowPacketPayload = (document.getElementById("server.game.isShowPacketPayload") as HTMLInputElement)?.checked || config.server.game.isShowPacketPayload;
        config.server.game.isShowLoopPackets = (document.getElementById("server.game.isShowLoopPackets") as HTMLInputElement)?.checked || config.server.game.isShowLoopPackets;
        config.server.game.gameOptions.inventoryLimits.weapons = parseInt((document.getElementById("server.game.gameOptions.inventoryLimits.weapons") as HTMLInputElement)?.value) || config.server.game.gameOptions.inventoryLimits.weapons;
        config.server.game.gameOptions.inventoryLimits.relics = parseInt((document.getElementById("server.game.gameOptions.inventoryLimits.relics") as HTMLInputElement)?.value) || config.server.game.gameOptions.inventoryLimits.relics;
        config.server.game.gameOptions.inventoryLimits.materials = parseInt((document.getElementById("server.game.gameOptions.inventoryLimits.materials") as HTMLInputElement)?.value) || config.server.game.gameOptions.inventoryLimits.materials;
        config.server.game.gameOptions.inventoryLimits.furniture = parseInt((document.getElementById("server.game.gameOptions.inventoryLimits.furniture") as HTMLInputElement)?.value) || config.server.game.gameOptions.inventoryLimits.furniture;
        config.server.game.gameOptions.inventoryLimits.all = parseInt((document.getElementById("server.game.gameOptions.inventoryLimits.all") as HTMLInputElement)?.value) || config.server.game.gameOptions.inventoryLimits.all;
        config.server.game.gameOptions.avatarLimits.singlePlayerTeam = parseInt((document.getElementById("server.game.gameOptions.avatarLimits.singlePlayerTeam") as HTMLInputElement)?.value) || config.server.game.gameOptions.avatarLimits.singlePlayerTeam;
        config.server.game.gameOptions.avatarLimits.multiPlayerTeam = parseInt((document.getElementById("server.game.gameOptions.avatarLimits.multiPlayerTeam") as HTMLInputElement)?.value) || config.server.game.gameOptions.avatarLimits.multiPlayerTeam;
        config.server.game.gameOptions.sceneEntityLimit = parseInt((document.getElementById("server.game.gameOptions.sceneEntityLimit") as HTMLInputElement)?.value) || config.server.game.gameOptions.sceneEntityLimit;
        config.server.game.gameOptions.watchGachaConfig = (document.getElementById("server.game.gameOptions.watchGachaConfig") as HTMLInputElement)?.checked || config.server.game.gameOptions.watchGachaConfig;
        config.server.game.gameOptions.enableShopItems = (document.getElementById("server.game.gameOptions.enableShopItems") as HTMLInputElement)?.checked || config.server.game.gameOptions.enableShopItems;
        config.server.game.gameOptions.staminaUsage = (document.getElementById("server.game.gameOptions.staminaUsage") as HTMLInputElement)?.checked || config.server.game.gameOptions.staminaUsage;
        config.server.game.gameOptions.energyUsage = (document.getElementById("server.game.gameOptions.energyUsage") as HTMLInputElement)?.checked || config.server.game.gameOptions.energyUsage;
        config.server.game.gameOptions.fishhookTeleport = (document.getElementById("server.game.gameOptions.fishhookTeleport") as HTMLInputElement)?.checked || config.server.game.gameOptions.fishhookTeleport;
        ...
        config.server.game.joinOptions.welcomeMail.sender = (document.getElementById("server.game.joinOptions.welcomeMail.sender") as HTMLInputElement)?.value || config.server.game.joinOptions.welcomeMail.sender;
        config.server.game.joinOptions.welcomeMail.items = this.state.mailItems || config.server.game.joinOptions.welcomeMail.items;
        config.server.game.serverAccount.avatarId = parseInt((document.getElementById("server.game.serverAccount.avatarId") as HTMLInputElement)?.value) || config.server.game.serverAccount.avatarId;
        config.server.game.serverAccount.nameCardId = parseInt((document.getElementById("server.game.serverAccount.nameCardId") as HTMLInputElement)?.value) || config.server.game.serverAccount.nameCardId;
        config.server.game.serverAccount.adventureRank = parseInt((document.getElementById("server.game.serverAccount.adventureRank") as HTMLInputElement)?.value) || config.server.game.serverAccount.adventureRank;
        ...
        config.server.debugMode.isShowPacketPayload = (document.getElementById("server.debugMode.isShowPacketPayload") as HTMLInputElement)?.checked || config.server.debugMode.isShowPacketPayload;
        config.server.debugMode.isShowLoopPackets = (document.getElementById("server.debugMode.isShowLoopPackets") as HTMLInputElement)?.checked || config.server.debugMode.isShowLoopPackets;
        config.server.debugMode.logRequests = (document.getElementById("server.debugMode.logRequests") as HTMLSelectElement)?.options[(document.getElementById("server.debugMode.logRequests") as HTMLSelectElement)?.selectedIndex ?? 0].value as ServerDebugMode || config.server.debugMode.logRequests;
    ...
    render() {
        return (
            <>
                    ...
                    <div className={"ConfigGen_Container"}>
                        ...
                        <div className={"ConfigGen_Config"}>
                            ...
                            <ConfigSection title={"Server"}>
                                ...
                                >
                                    <select id={"server.runMode"} defaultValue={"HYBRID"}>
                                ...
                                >
                                    <input
                                        type={"number"}
                                        placeholder={"100"}
                                        id={"server.game.loadEntitiesForPlayerRange"}
                                    />
                                </ConfigField>
                                ...
                                >
                                    <input
                                        type={"checkbox"}
                                        id={"server.game.enableScriptInBigWorld"}
                                    />
                                </ConfigField>
                                ...
                                >
                                    <select id={"server.game.logPackets"} defaultValue={"NONE"}>
                                ...
                                >
                                    <input
                                        type={"number"}
                                        placeholder={"1000"}
                                        id={"server.game.gameOptions.sceneEntityLimit"}
                                    />
                                </ConfigField>
                                ...
                                >
                                    <input
                                        type={"checkbox"}
                                        defaultChecked={true}
                                        id={"server.game.gameOptions.energyUsage"}
                                    />
                                </ConfigField>
                                ...
                                >
                                    <input
                                        type={"checkbox"}
                                        defaultChecked={true}
                                        id={"server.game.gameOptions.fishhookTeleport"}
                                    />
                                </ConfigField>
                                ...
                                >
                                    <input
                                        type={"checkbox"}
                                        id={"server.game.gameOptions.resinOptions.resinUsage"}
                                    />
                                </ConfigField>
                                ...
                                >
                                    <input
                                        type={"number"}
                                        placeholder={"160"}
                                        id={"server.game.gameOptions.resinOptions.cap"}
                                    />
                                </ConfigField>
                                ...
                                >
                                    <input
                                        type={"number"}
                                        placeholder={"480"}
                                        id={"server.game.gameOptions.resinOptions.rechargeTime"}
                                    />
                                </ConfigField>
                                ...
                                >
                                    <BasicButton
                                        ...
                                        id={"config-add-item"}/>}
                                        onClick={() => BasicModal.showModal("config-add-item-modal")}
                                    />

                                    {
                                        this.state.mailItems.length > 0 ?
                                            <BasicButton
                                                ...
                                                color={"#d2592c"}
                                                onClick={() => BasicModal.showModal("config-mail-list-modal")}
                                            />
                                            : null
                                    }
                                </ConfigField>
                                ...
                                >
                                    <BasicButton
                                        ...
                                        onClick={() => BasicModal.showModal("config-add-region-modal")}
                                    />

                                    {
                                        this.state.regions.length > 0 ?
                                            <BasicButton
                                                ...
                                                color={"#d2592c"}
                                                onClick={() => BasicModal.showModal("config-region-list-modal")}
                                            />
                                            : null
                                    }
                                </ConfigField>
                                ...
                                >
                                    <select id={"server.dispatch.logRequests"} defaultValue={"NONE"}>
                                        <option value={"ALL"}>ALL</option>
                                        <option value={"MISSING"}>
                                            MISSING
                                        </option>
                                ...
                                >
                                    <select id={"server.debugMode.serverLoggerLevel"} defaultValue={"DEBUG"}>
                                ...
                                >
                                    <select id={"server.debugMode.servicesLoggerLevel"} defaultValue={"INFO"}>
                                        <option value={"OFF"}>OFF</option>
                                        <option value={"ERROR"}>ERROR</option>
                                        <option value={"WARN"}>WARN</option>
                                        <option value={"INFO"}>INFO</option>
                                ...
                                >
                                    <select id={"server.debugMode.logPackets"} defaultValue={"ALL"}>
                                        <option value={"ALL"}>ALL</option>
                                        <option value={"MISSING"}>
                                            MISSING
                                        </option>
                                ...
                                >
                                    <input
                                        type={"checkbox"}
                                        id={"server.debugMode.isShowPacketPayload"}
                                    />
                                </ConfigField>
                                ...
                                >
                                    <input
                                        type={"checkbox"}
                                        id={"server.debugMode.isShowLoopPackets"}
                                    />
                                </ConfigField>
                                ...
                                >
                                    <select id={"server.debugMode.logRequests"} defaultValue={"ALL"}>
                                        <option value={"ALL"}>ALL</option>
                                        <option value={"MISSING"}>
                                            MISSING
                                        </option>
                ...
                <BasicModal id={"config-add-item-modal"} onSubmit={() => this.addListItem("MailItem")} buttonText={"Add"}>
                    <p>Item ID</p>
                    <input type={"number"} id={"config-add-item-id"} placeholder={"10000001"}/>
                    <p>Item Count</p>
                    <input type={"number"} id={"config-add-item-count"} placeholder={"1"}/>
                    <p>Item Level</p>
                    <input type={"number"} id={"config-add-item-level"} placeholder={"1"}/>
                </BasicModal>

                <BasicModal id={"config-add-region-modal"} onSubmit={() => this.addListItem("Region")} buttonText={"Add"}>
                    <p>Region Name</p>
                    <input type={"text"} id={"config-add-region-name"} placeholder={"Grasscutter"}/>
                    <p>Region Title</p>
                    <input type={"text"} id={"config-add-region-title"} placeholder={"Grasscutter"}/>
                    <p>Region IP Address</p>
                    <input type={"text"} id={"config-add-region-ip"} placeholder={"127.0.0.1"}/>
                    <p>Region Port</p>
                    <input type={"number"} id={"config-add-region-port"} placeholder={"9000"}/>
                </BasicModal>

                <BasicModal id={"config-mail-list-modal"}>
                    <div className={"config-mail-list"}>
                        {this.state.mailItems.map((item, index) => (
                            <ListItem type={"MailItem"} item={item} onDelete={() => this.deleteListItem(item)} key={index} />
                        ))}
                    </div>
                </BasicModal>
                ...
                <BasicModal id={"config-region-list-modal"}>
                    <div className={"config-region-list"}>
                        {this.state.regions.map((region, index) => (
                            <ListItem type={"Region"} item={region} onDelete={() => this.deleteListItem(region)} key={index} />
                        ))}
                    </div>
                </BasicModal>
            </>
